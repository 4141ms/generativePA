...
from utils.losses import AutomaticWeightedLoss, MIPloss
...
class BrownianBridgeModel(nn.Module):
    ...
    def forward_muti(self, batch):
        ...
        x_t_1, _ = self.q_sample(x0, batch['T1'], t, noise)
        x_t_2, _ = self.q_sample(x0, batch['T2'], t, noise)
        x_t_3, _ = self.q_sample(x0, batch['MRA'], t, noise)


        objective_recon_1 = self.denoise_fn_1(x_t_1, time_emb=t_emb, context=batch['T1'])
        objective_recon_2 = self.denoise_fn_2(x_t_2, time_emb=t_emb, context=batch['T2'])
        objective_recon_3 = self.denoise_fn_2(x_t_3, time_emb=t_emb, context=batch['MRA'])

        x0_1 = self.predict_x0_from_objective(x_t_1, batch['T1'], t, objective_recon=objective_recon_1)
        x0_2 = self.predict_x0_from_objective(x_t_2, batch['T2'], t, objective_recon=objective_recon_2)
        x0_3 = self.predict_x0_from_objective(x_t_3, batch['MRA'], t, objective_recon=objective_recon_3)

        feat_1 = self.cross1(torch.cat([x0_1, batch['T2'], batch['MRA']], 1))
        feat_2 = self.cross2(torch.cat([x0_2, batch['T1'], batch['MRA']], 1))
        feat_3 = self.cross3(torch.cat([x0_3, batch['T1'], batch['T2']], 1))
    ...
    def sample(self, batch, context=None, clip_denoised=True, sample_mid_step=False):
        ...
        for i in tqdm(range(len(self.steps)), desc=f'sampling loop time step', total=len(self.steps)):
            ...
            x0_1 = self.p_sample_new(x_t=x_t_1, y=batch['T1'], t_emb=t_emb,
                                      t=t, clip_denoised=clip_denoised, fn=self.denoise_fn_1)
            x0_2 = self.p_sample_new(x_t=x_t_2, y=batch['T2'], t_emb=t_emb,
                                      t=t, clip_denoised=clip_denoised, fn=self.denoise_fn_2)
            x0_3 = self.p_sample_new(x_t=x_t_3, y=batch['MRA'], t_emb=t_emb,
            ...
            x0_recon = self.fusion(input, t_emb)
            x_t_1 = self.get_xtminus(x_t_1, noise, t, batch['T1'], x0_recon, i)
            x_t_2 = self.get_xtminus(x_t_2, noise, t, batch['T2'], x0_recon, i)
            x_t_3 = self.get_xtminus(x_t_3, noise, t, batch['MRA'], x0_recon, i)
    ...
    def sample_faster(self, batch, context=None, clip_denoised=True, sample_mid_step=False):
        ...
        for i in tqdm(range(len(self.steps)), desc=f'sampling loop time step', total=len(self.steps)):
            ...
            x_t_1 = self.get_xtminus(x_t_1, noise, t, batch['T1'], x0_1, i)
            x_t_2 = self.get_xtminus(x_t_2, noise, t, batch['T2'], x0_2, i)
            x_t_3 = self.get_xtminus(x_t_3, noise, t, batch['MRA'], x0_3, i)
