...
def print_gpu_memory_report():
    if torch.cuda.is_available():
        ...
        # 获取当前使用的 GPU 索引（PyTorch 默认使用的 GPU）
        gpu_index = torch.cuda.current_device()  # 例如：0（第一张卡）
        ...
        gpu_utilization = util.gpu  # GPU 利用率

        # 获取显存信息（单位：字节，需转换为 MB）
        mem_info = pynvml.nvmlDeviceGetMemoryInfo(handle)
        memory_used = mem_info.used / (1024 ** 2)  # 已用显存（MB）
        memory_total = mem_info.total / (1024 ** 2)  # 总显存（MB）

        # 打印结果（与原代码功能一致）
        print("Memory report")
        print(f"GPU 利用率: {gpu_utilization}%")
        print(f"已用显存: {memory_used:.2f} MB / 总显存: {memory_total:.2f} MB")

        # 关闭 NVML（可选，程序结束时自动释放）
        pynvml.nvmlShutdown()
