...
class ResnetGenerator_ncsn(nn.Module):
    ...
    def __init__(self, input_nc, cond_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9,
                 padding_type='reflect',  no_antialias=False, no_antialias_up=False, opt=None):
        """Construct a Resnet-based generator

        Parameters:
            input_nc (int)      -- the number of channels in input images
            cond_nc (int)      -- the number of channels in condition
        ...
        model = [nn.ReflectionPad2d(3),
                 nn.Conv2d(input_nc + cond_nc, ngf, kernel_size=7, padding=0, bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        self.ngf = ngf
        n_downsampling = 2
    ...
    def forward(self, x, time_cond,z, layers=[], condition=None, encode_only=False):
