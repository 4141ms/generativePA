...
class SBModel(BaseModel, nn.Module):
    @staticmethod
    ...
    def set_input(self, input, input2=None, condition=None):
        ...
        self.real_B = input2.to(self.device)
        self.condition = condition.to(self.device) if condition is not None else None
    ...
    def calculate_NCE_loss(self, src, tgt):
        ...
        feat_q = self.netG(tgt, self.time_idx * 0, z, self.nce_layers, self.condition,encode_only=True)

        if self.opt.flip_equivariance and self.flipped_for_equivariance:
            feat_q = [torch.flip(fq, [3]) for fq in feat_q]

        feat_k = self.netG(src,  self.time_idx * 0, z, self.nce_layers, self.condition,encode_only=True)
